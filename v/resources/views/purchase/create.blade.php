...
<section class="content">
	...
	@component('components.widget', ['class' => 'box-primary'])
		{!! Form::hidden('is_bakery',$_is_bakery) !!}
					...
					<div class="col-sm-4">
						<div class="form-group">
							<div class="input-group">
								...
								@endphp
								{!! Form::hidden('hidden_price_group',!is_null($default_price_group_id) ? $default_price_group_id :  key($price_groups), ['id' => 'hidden_price_group']) !!}
								{!! Form::select('price_group', $price_groups, $selected_price_group, ['class' => 'form-control select2', 'id' => 'price_group', 'required',]); !!}
								<span class="input-group-addon">
										@show_tooltip(__('lang_v1.purchase_price_group_help_text'))
									</span>
							</div>
						</div>
					</div>
		...
		<div class="row">
			...
			@if(isset($warehouse) && !empty($warehouse))
				@if(count($business_locations) ==1)
					@php
                        $default_location = current(array_keys($bl_locations->toArray()));
                        $search_disable = true;
					@endphp
				@else
					@php
						$default_location = $warehouse->name;
                        $search_disable = true;
					@endphp
				@endif
			@else
			...
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('location_id', $bl_locations, $default_location, ['class' => 'form-control select2',  count($bl_locations) == 1 ? "" :'placeholder' =>  __('messages.please_select') , 'required'], $bl_attributes); !!}
...
@section('javascript')
	...
	<script type="text/javascript">
		...
		$('#price_group').on('change', function () {
			$('#hidden_price_group').val($(this).val())
		})
	</script>
