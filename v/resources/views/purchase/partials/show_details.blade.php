...
<div class="modal-body">
  ...
  @if($purchase->type == 'purchase_order'
            ...
            && auth()->user()->can('purchase_order.confirm')
            && !empty($common_settings['enable_confirm_order'])
            )
    <div class="row" style="display:flex;justify-content:end;margin-right: 0.8rem">
      <button type="button"  id="confirm_order" class="btn btn-info ladda-button">@lang( 'Confirm Order' )</button>
    </div>
  @endif
  <br>
  <div class="row">
        ...
        <table class="table bg-gray">
          ...
          @foreach($purchase->purchase_lines as $purchase_line)
            ...
            @php 
              ...
              $total_tax += ($purchase_line->item_tax * $purchase_line->quantity);
              $total_final += ($purchase_line->purchase_price_inc_tax * $purchase_line->quantity);
            @endphp
          @endforeach
        </table>
      </div>
      ...
      <div class="table-responsive">
        <table class="table">
          ...
          <tr>
            ...
            <td></td>
            <td><span class="display_currency pull-right" data-currency_symbol="true" >{{ $total_final }}</span></td>
          </tr>
        </table>
      </div>
    </div>
  ...
  <script>
    $(document).on('click', '#confirm_order', function (e) {
      swal({
        title: LANG.sure,
        text: 'Do you want to confirm this order as correct?',
        icon: 'warning',
        dangerMode: true,
        buttons : true
      }).then(confirm => {
        if (confirm) {
          ...
          $.ajax({
            method : "PUT",
            url : "{{ action([\App\Http\Controllers\PurchaseOrderController::class,'postOrderConfirmation'], ['id' => $purchase->id]) }}",
            data : {
              status : 'confirmed'
            },
            success : (res) => {
