@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            @php
                $purchase_price = !empty($purchase_order_line) ? $purchase_order_line->purchase_price/$purchase_order->exchange_rate : $variation->default_purchase_price;
                 ...
                 //Get the group price if any
                $group_price = isset($relationship_exists) && $relationship_exists && !empty($variation->group_prices->first()) ? $variation->group_prices[0] : null;

                //Purchasing price for group price
                $purchase_gr_price = !is_null($group_price) &&  $group_price->purchase_price_inc_tax != 0
                        ? $group_price->purchase_price_inc_tax : $purchase_price;
                ...
                ? $purchase_order_line->pp_without_discount/$purchase_order->exchange_rate
                    : ($purchase_gr_price == $purchase_price
                        ? $variation->default_purchase_price
                        : ($tax_id == 0 ? $purchase_gr_price : $purchase_gr_price/1.18));
            ...
            @endphp
            @php
                $_permission_disabled_attr = 'readonly';
            @endphp
            {{-- Purchase Order Line (Used when a purchase is made from existing orders --}}
            @if(empty($purchase_order_line))
                @if($permit_price_editing == \App\Enums\PermissionEditing::IS_PERMITTED || auth()->user()->can('purchase.create'))
                    @php
                        $_permission_disabled_attr = '';
                    @endphp
                @endif
            @elseif(auth()->user()->can('purchase.create'))
                @php
                    $_permission_disabled_attr = '';
                @endphp
            @endif
            ...
            number_format($pp_without_discount, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm purchase_unit_cost_without_discount input_number', 'required', $_permission_disabled_attr]); !!}
        ...
        <td>
            {!! Form::text('purchases[' . $row_count . '][discount_percent]', number_format($discount_percent, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm inline_discounts input_number', 'required',$_permission_disabled_attr]); !!}
        ...
        <td>
            ...
            number_format($purchase_gr_price != $purchase_price ? ($tax_id == 0 ? $purchase_gr_price : $purchase_gr_price/1.18) : $purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm purchase_unit_cost input_number', 'required',$_permission_disabled_attr]); !!}
        </td>
        <td class="{{$hide_tax}}">
            ...
            @php
                ...
                if($hide_tax == 'hide'){
                 ...
                 $dpp_inc_tax = !empty($purchase_order_line)
                                ...
                                : (!is_null($group_price) && $purchase_gr_price != $purchase_price ? $purchase_gr_price :  $dpp_inc_tax);

            @endphp
            {!! Form::text('purchases[' . $row_count . '][purchase_price_inc_tax]', $dpp_inc_tax, ['class' => 'form-control input-sm purchase_unit_cost_after_tax input_number', 'required', $_permission_disabled_attr]); !!}
        </td>
        <td>
        ...
        <td class="@if(!session('business.enable_editing_product_from_purchase') || !empty($is_purchase_order)) hide @endif">
            {!! Form::text('purchases[' . $row_count . '][profit_percent]', number_format($variation->profit_percent, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number profit_percent', 'required', $_permission_disabled_attr]); !!}
        </td>
        @if(empty($is_purchase_order))
        <td>
            @if(session('business.enable_editing_product_from_purchase'))
                {!! Form::text('purchases[' . $row_count . '][default_sell_price]', number_format(!is_null($group_price) && $group_price->sell_price_inc_tax != 0 ? $group_price->sell_price_inc_tax : $variation->sell_price_inc_tax, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm input_number default_sell_price', 'required', $_permission_disabled_attr]); !!}
            @else
