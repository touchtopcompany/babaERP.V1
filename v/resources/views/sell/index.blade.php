            ...
            @slot('tool')
                <div class="box-tools" style="display: flex">
                    ...
                    <i class="fa fa-plus"></i> @lang('messages.add')</a>
                      <button type="button" style="padding: 2px 12px;margin-left: 0.8rem" class="btn btn-block btn-success btn-modal pull-riht"
                            data-href="{{action([\App\Http\Controllers\ReportController::class,'getSellsSummaryReport'])}}"
                            data-container=".view_modal" data-bcup-haslogintext="no">
                        <i class="fa fa-cloud-upload-alt"></i> @lang('messages.print_sells_report')</button>
                </div>
            @endslot
        @endcan
            ...
            <table class="table table-bordered table-striped ajax_view" id="sell_table">
                ...
                <tfoot>
                    <tr class="bg-gray font-17 footer-total text-center">
                        ...
                        <td colspan="2" class="footer_total_remaining"></td>
   ...
   $('#product_sr_start_time, #product_sr_end_time').datetimepicker({
    ...
    sell_table = $('#sell_table').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            if(data.length > 0){
                if(data[r].status_of_payment === 'outstanding'){
                    $('.footer_total_remaining').css('font-size', '1.8rem').html("<span class='label bg-red'> Outstanding Balance = " + __currency_trans_from_en(footer_total_remaining) + "</span>");
                }
            }
