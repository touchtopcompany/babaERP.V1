...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	<td>
		...
		@php
            ...
            $item_tax = !empty($product->tax_id) ? $product->sell_price_inc_tax - $product->default_sell_price : 0;
			...
			<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-sm sub_unit">
                @foreach($sub_units as $key => $value)
					@php
						...
						$calculate_multiplier = (new \App\Utils\ProductUtil())->calculateProductMultiplier($business_id, $product, $value,$_is_product);
                        $product_multi = 1;
                        if($calculate_multiplier != 1)
                        {
                        $product_multi = $calculate_multiplier;
                        }
					@endphp
                    <option value="{{$key}}" data-multiplier="{{($product_multi)}}" data-unit_name="{{$value['name']}}" data-allow_decimal="{{$value['allow_decimal']}}" @if(!empty($product->sub_unit_id) && $product->sub_unit_id == $key) selected @endif>
                        {{$value['name']}}
                    </option>
                @endforeach
           </select>
