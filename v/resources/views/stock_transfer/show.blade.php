<div class="modal-dialog modal-xl" role="document">
	...
	<div class="modal-content">
		<div class="modal-header">
		    ...
		    <h4 class="modal-title" id="modalTitle">
				@if(!$not_previous_trans)
					@lang('Previous Transfer')
				@else
					@lang('lang_v1.stock_transfer_details')
				@endif
				(<b>@lang('purchase.ref_no'):</b> #{{ $sell_transfer->ref_no }})
		    </h4>
		</div>
				...
				<div class="row invoice-info">
				  ...
				  <div class="col-sm-4 invoice-col">
				    ...
				    <b>@lang('sale.status'):</b> {{$statuses[$sell_transfer->status] ?? ''}}
					  @if(!empty($previous_trans))
						  <div class="f-right">
							  <button class="btn-warning btn btn-block reduce-size show_previous_modal" data-href="{{ action([\App\Http\Controllers\StockTransferController::class, 'showPreviousTransfer'], [$previous_trans->id])}}">Original Transfer</button>
						  </div>
					  @endif
				  </div>
				</div>
	...
	<script>
		$(document).on('click', '.show_previous_modal', function () {
