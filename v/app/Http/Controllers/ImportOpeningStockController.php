                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (! empty($value[0])) {
                        ...
                        $product_info = Variation::where('sub_sku', $sku)
                                ...
                                ->select(['P.id','P.name as name', 'variations.id as variation_id',
                    ...
                    //Never added opening stock more than once
                    if(is_null($os_transaction))
                    {
                        $this->addOpeningStock($opening_stock, $product_info, $business_id, $unit_cost_before_tax, $os_transaction);
                        
                         $log_properties = [
                            'name' => $product_info->name,
                            'opening_stock' => trim($value[2]),
                            'location' => $location->name
                        ];
                        $this->productUtil->activityLog($product_info, 'opening_stock', null, $log_properties);
