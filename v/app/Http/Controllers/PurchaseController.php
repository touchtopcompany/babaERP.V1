...
use App\Enums\PermissionEditing;
...
class PurchaseController extends Controller
{
    ...
    protected const BAKERY = 'Bakery';
    protected const PANONE_SUPPLIER = 'PANONE MIN SUPERMARKET';
    ...
    public function index()
    {
        ...
        $bakery_location = null;
        if ($permitted_locations != 'all') {
            foreach ($permitted_locations as $permitted_location) {
                $location_name = BusinessLocation::getLocationName($permitted_location);
                if (Str::contains($location_name, self::BAKERY)) {
                    ...
                    $bakery_location = $permitted_location;
                    break;
                }
            }
        }
        ...
        if (request()->ajax()) {
            ...
            if (empty(request()->location_id)) {
                if (!is_null($bakery_location)) {
                    $purchases->where('contacts.supplier_business_name', self::PANONE_SUPPLIER);
            ...
            if (!empty(request()->location_id)) {
                ...
                if (!is_null($bakery_location) && request()->location_id != $bakery_location) {
                    $purchases->where('contacts.supplier_business_name', self::PANONE_SUPPLIER);
                }
            }
            ...
            return Datatables::of($purchases)
                ...
                ->addColumn('received_by', function ($row){
                    $changes = json_decode($row->user_changes);

                    $received_by = "NOT RECORDED";
                    if ($row->status == TransactionStatus::RECEIVED && !empty($changes->user)) {
                        $user = User::query()->findOrFail($changes->user);
                        $received_by = $user->surname . ' ' . $user->first_name . ' ' . $user->last_name;
                    }
                    return $received_by;
                })
    ...
    public function create()
    {
        ...
        $taxes = TaxRate::where('business_id', $business_id)
            ...
            ->get();
        $orderStatuses = $this->productUtil->orderStatuses();
        $price_groups = PriceGroup::forDropdown($business_id);

        $permitted_locations = auth()->user()->permitted_locations();
        $business_locations = BusinessLocation::forDropdown($business_id, false, true,false);
        $bl_locations = $business_locations['locations'];
        $bl_attributes = $business_locations['attributes'];

        $_is_bakery = BakeryLocation::IS_NOT_BAKERY;
        if (count($bl_locations) == 1 && $permitted_locations != 'all') {
            //Get the string from the array
            $location_name = implode('',$bl_locations->toArray());
            if (Str::contains($location_name, self::BAKERY)) {
                $_is_bakery = BakeryLocation::IS_BAKERY;
                $business_locations = BusinessLocation::forDropdown($business_id,false,true,false,false);
                $bl_locations = $business_locations['locations'];
                $bl_attributes = $business_locations['attributes'];
            }
        }
        ...
        $warehouse = BusinessLocation::where([
            'business_id' => $business_id,
        ])->where('name','like', '%ware%')->first(['id','name']);
        ...
        return view('purchase.create')
            ->with(compact('taxes', 'orderStatuses', 'business_locations', 'warehouse', 'price_groups','default_price_group_id','_is_bakery', 'currency_details', 'default_purchase_status', 'customer_groups', 'types', 'shortcuts', 'payment_line', 'payment_types', 'accounts', 'bl_attributes','bl_locations', 'common_settings'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            //Purposely for Bakery (Panone) to perform stock deduction
            if((new BakeryRequestTransactionAction())->handle($request)) {
                // (new StockTransferController($this->productUtil, $this->transactionUtil, $this->moduleUtil))->store($request, $transaction);
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if($update_data['status'] == TransactionStatus::RECEIVED) {
                $update_data['user_changes'] = json_encode([
                    'user' => auth()->user()->id,
                    'changes' => TransactionStatus::RECEIVED
                ]);
            }
    ...
    public function getPurchaseEntryRow(Request $request)
    {
        if (request()->ajax()) {
            ...
            if (!empty($product_id)) {
                ...
                //Check if the location has been sat a selling and purchasing group

                //To check if price can be edited or not
                $permit_price_editing = PermissionEditing::IS_NOT_PERMITTED;
                $relationship_exists = false;
                if(auth()->user()->can('edit_product_price_from_purchase_screen'))
                {

                    //Is permitted from permission
                    $permit_price_editing = PermissionEditing::IS_PERMITTED;
                    $price_group_count = PriceGroup::countPriceGroups($business_id);
                    if ($price_group_count > 0) {

                        //Any location having selling price group
                        $location_price_group_count = BusinessLocation::query()->whereNotNull('selling_price_group_id')->count();
                        if ($location_price_group_count > 0) {
                            //Editing Not Allowed
                            $permit_price_editing = PermissionEditing::IS_NOT_PERMITTED;

                            //Having default set price
                            $group_price_id = BusinessLocation::query()->findOrFail($location_id)->selling_price_group_id;
                            ...
                            if (!is_null($group_price_id) && ($group_price_id > 0)) {
                                ...
                                //Get price groups
                                $query->with([
                                    'group_prices' => function ($q) use ($group_price_id, $variation_id) {
                                        $q->where([
    ...
    public function updateStatus(Request $request)
    {
        ...
        try {
            ...
            $update_data['user_changes'] = json_encode(['user' => auth()->user()->id, 'changes' =>  $update_data['status']]);
