...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        //Package modules
        $package_mods = (new ModulesController($this->moduleUtil))->__available_modules();
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules','package_mods', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types'));
    }
        ...
        public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'min_redeem_point', 'max_redeem_point', 'rp_expiry_period',
                'rp_expiry_type', 'custom_labels', 'weighing_scale_setting',
                'code_label_1', 'code_1', 'code_label_2', 'code_2', 'currency_precision', 'quantity_precision', 'pricing_profit_strategy']);
            ...
            $request->session()->put('financial_year', $financial_year);
                ...
                //Purchased One
                $purchased_mods = (new ModulesController($this->moduleUtil))->__purchased_modules();

                $active_modules = [];
                  if(!empty($enabled_modules)){
                foreach ($enabled_modules as $enabled_module) {
                    if(in_array($enabled_module, $purchased_mods )){
                        $active_modules[] = $enabled_module;
                    }
                }
                  }

                //Register this modules on the main domain
                //Get core domain connection
                $core = DB::connection('core_domain');

                //Check for valid connection with the database
                @mysqli_connect(
                    username: $core->getConfig('username'),
                    password: $core->getConfig('password'),
                    database: $core->getConfig('database'),
                );
                //Everything is fine
                DB::beginTransaction();
                  //Get current subdomain
                $sub_domain = $_SERVER['HTTP_HOST'];
                $core->table('subdomains')
                ...
                $package_mods = unserialize((new ModulesController($this->moduleUtil))->__available_modules());
                foreach ($package_mods as $details) {
                    $name = $details['n'];
                    if (in_array(Str::lower($name), $active_modules)) {
                        //Install the module for this domain
                        $is_installed = System::getProperty(Str::lower($name) . '_version');
                        ...
                        System::addProperty(Str::lower($name) . '_version',  config(Str::lower($name).'.module_version'));
                    }
                }
