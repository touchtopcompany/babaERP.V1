...
class TransactionController extends Controller
{
                    ...
                    function ($row) {
                        ...
                        if (auth()->user()->can('accounting.map_transactions')) {
                            ...
                            if (! $is_mapped) {
                                ...
                                $html .= '<a data-href="'.action([\Modules\Accounting\Http\Controllers\TransactionController::class, 'destroyMap']).'?id='.$row->id.'" class="btn-modal btn btn-danger btn-xs mt-10 delete-mapping"><i class="fas fa-trash"></i> '.__('accounting::lang.delete_mapping').'</a>';
                ...
                ->addColumn('action', function ($row) use ($transaction_type) {
                    ...
                    if (auth()->user()->can('accounting.map_transactions')) {
                        ...
                        if (! $is_mapped) {
                            ...
                            $html .= '<a data-href="'.action([\Modules\Accounting\Http\Controllers\TransactionController::class, 'destroyMap']).'?payment_id='.$row->transaction_payment_id.'" class="btn-modal btn btn-danger btn-xs mt-10 delete-mapping"><i class="fas fa-trash"></i> '.__('accounting::lang.delete_mapping').'</a>';
    ...
    {
        if($request->has('id')) {
            $this->accountingUtil->deleteMap($request->id, null);
        }
        elseif($request->has('payment_id')){
            $this->accountingUtil->deleteMap(null, $request->payment_id);
        }

        return ['success' => true,
                 'msg' => __('accounting::lang.successfully_deleted'),
        ];
    }
