...
<div class="row">
    ...
    @component('components.widget', ['title' => $product->name])
        ...
        <div class="col-md-4">
            <div class="form-grou pull-right">
                <div class="input-group">
                    <button type="button" class="btn btn-primary" id="stock_history_date_filter">
                    <span>
...
@section('javascript')
   <script type="text/javascript">
        $(document).ready( function(){
            ...
            $('#stock_history_date_filter').change(function() {
                load_stock_history($('#variation_id').val(), $('#location_id').val());
            });
        });

        if ($('#stock_history_date_filter').length == 1) {
            $('#stock_history_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
                $('#stock_history_date_filter span').html(
       ...
       function load_stock_history(variation_id, location_id) {
           ...
           var start = $('#stock_history_date_filter')
               .data('daterangepicker')
               .startDate.format('YYYY-MM-DD');
           var end = $('#stock_history_date_filter')
               .data('daterangepicker')
               .endDate.format('YYYY-MM-DD');

            $.ajax({
                url: '/products/stock-history/' + variation_id + "?location_id=" + location_id + "&start_date=" + start + "&end_date=" + end,
                dataType: 'html',
