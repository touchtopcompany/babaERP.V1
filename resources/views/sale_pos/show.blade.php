...
<div class="modal-body">
    ...
    <div class="row">
      ...
      @endif
       @if($sell->type == 'sales_order' && !in_array($sell->status, ['confirmed','completed']) && auth()->user()->can('so.confirm'))
     ...
     @if($sell->type == 'sales_order')
      ...
      swal({
        ...
        if (confirm) {
          ...
          $.ajax({
            method : "PUT",
            url : "{{ action([\App\Http\Controllers\SalesOrderController::class,'postOrderConfirmation'], ['id' => $sell->id]) }}",
            data : {
              status : 'confirmed'
            },
            success : (res) => {
