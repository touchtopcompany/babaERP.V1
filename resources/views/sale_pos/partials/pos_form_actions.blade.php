...
<div class="row">
	<div class="pos-form-actions">
		<div class="col-md-12">
			...
			@endif
			<button type="button" class="@if($is_mobile) col-xs-6 @endif btn bg-info no-print text-white btn-default btn-flat @if($pos_settings['disable_draft'] != 0) hide @endif" id="pos-draft" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('sale.draft')</button>
			<button type="button" class="btn btn-default bg-yellow btn-flat no-print @if($is_mobile) col-xs-6 @endif" id="pos-quotation" @if(!empty($only_payment)) disabled @endif><i class="fas fa-edit"></i> @lang('lang_v1.quotation')</button>
			...
			@if(empty($pos_settings['disable_credit_sale_button']))
				...
				title="@lang('lang_v1.tooltip_credit_sale')" @if(!empty($only_payment)) disabled @endif>
					...
					@if( $_SERVER['HTTP_HOST'] == 'lushgardenhotel.babaerp.live')
					    {{ 'Bill' }}
					@else
			...
			@if(empty($edit))
				<button type="button" class="btn btn-danger no-print btn-flat @if($is_mobile) col-xs-6 @else btn-xs @endif" id="pos-cancel"> <i class="fas fa-window-close"></i> @lang('sale.cancel')</button>
			@else
			...
			@if(!$is_mobile)
			<div class="bg-navy pos-total text-white no-print">
			...
			<button type="button" class="pull-right btn no-print btn-primary btn-flat @if($is_mobile) col-xs-6 @endif" data-toggle="modal" data-target="#recent_transactions_modal" id="recent-transactions"> <i class="fas fa-clock"></i> @lang('lang_v1.recent_transactions')</button>
			@endif
