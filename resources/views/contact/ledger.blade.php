...
<div class="col-md-12 col-sm-12 @if(!empty($for_pdf)) width-100 align-right @endif">
	@if(!empty($location))
		@if(!empty($location->invoice_logo))
			<p class="align-right">
				<img alt="" class="width-40"  src="{!! nl2br(asset("uploads/invoice_logos/".$location->invoice_logo))!!}">
			</p>
		@endif
	@endif
	<p class="text-right align-right">
	@if(!empty($location))
				@if($contact->business->name == 'Umeme Plus Ltd')
					{{$location->name}}
				@else
...
<div  style="margin-top: 2.4rem" class="col-md-6 col-sm-6 col-xs-6 @if(!empty($for_pdf)) width-50 f-left @endif">
