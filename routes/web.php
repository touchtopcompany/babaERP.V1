...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin'])->group(function () {
    ...
    Route::get('sms-balance', [HomeController::class, 'getSMSBalance'])->name('sms-balance');

    // require_once'subdomain.php';
 Route::controller(MultiCurrencyController::class)->group(function () {
        Route::get('multi-currency-settings', 'getSettings')->name('multi-currency-settings');
        Route::get('conversion_exchange_rate_api', 'APIExchangeRate')->name('conversion-exchange-rate-api');

        //Update location session currency
        // Route::put('update_location_currency', 'setNewSessionCurrency');
    });
    Route::resource('multi-currency', MultiCurrencyController::class)->except(['show']);
    ...
    Route::get('/products/add-group-prices/{id}', [ProductController::class, 'addGroupPrices']);
    Route::post('/products/save-group-prices', [ProductController::class, 'saveGroupPrices']);
    ...
    Route::post('/sells/void-product', [SellPosController::class, 'addVoidProduct']);
    Route::get('/sells/get-void-product', [SellPosController::class, 'getVoidProduct'])->name('get-void-product');
    Route::get('/sells/check-transaction-status', [SellPosController::class, 'checkTransactionStatusBeforeEdit'])->name('check-transaction-status');

    Route::get('/reset-mapping', [SellController::class, 'resetMapping']);

    Route::get('/pos/create', [SellPosController::class, 'create'])->name('pos.create');
    ...
    Route::get('/reports/trending-products', [ReportController::class, 'getTrendingProducts']);
        Route::get('/reports/trending-products/print', [ReportController::class, 'printTrendingProducts'])->name('report.trending_products.print');
    ...
    Route::get('/reports/stock_evaluation', [ReportController::class, 'getStockEvaluationByDepartmentReport']);
        Route::get('/reports/sells-summary-report', [ReportController::class, 'getSellsSummaryReport']);
    ...
    Route::get('stock-transfers/previous-transfer/{id}', [StockTransferController::class, 'showPreviousTransfer']);
    ...
    //Stock taking
    Route::get('/stock-taking', [StockTakingController::class, 'getImportInventory'])->name('stock-taking.import');
    Route::post('/stock-taking', [StockTakingController::class, 'postImportInventory']);
    Route::post('/stock-taking/download-stock_taking-template', [StockTakingController::class,'downloadInventoryTemplate']);
    ...
    Route::get('price-group/activate-deactivate/{id}', [PriceGroupController::class, 'activateDeactivate']);
    Route::get('update-product-price', [PriceGroupController::class, 'updateProductPrice'])->name('update-product-price');
    Route::post('export-product-price', [PriceGroupController::class, 'export']);
    Route::post('import-product-price', [PriceGroupController::class, 'import']);

//    Route::get('price-group/activate-deactivate/{id}', 'PriceGroupController@activateDeactivate');
//    Route::get('export-price-group', 'PriceGroupController@export');
//    Route::post('import-price-group', 'PriceGroupController@import');
//
    Route::resource('price-group', PriceGroupController::class);
    ...
    Route::put('confirm-purchase-order/{id}/status', [PurchaseOrderController::class, 'postOrderConfirmation']);
    ...
    Route::put('update-sales-orders/{id}/status', [SalesOrderController::class, 'postEditSalesOrderStatus']);
        Route::put('confirm-sales-order/{id}/status', [SalesOrderController::class, 'postOrderConfirmation']);
...
Route::middleware(['setData', 'auth', 'SetSessionData', 'language', 'timezone'])->group(function () {
    ...
    Route::get('clear_log', function(){
        file_put_contents(storage_path('logs/laravel.log'),'');
    });
});


Route::get('/datetime-to-ticks/{datetime}', function($datetime){
    $timestamp = strtotime($datetime);
  $ticks = ($timestamp + 62135596800) * 10000000;
  return $ticks;
});
 Route::get('/tick-time/{ticks}', function ($ticks) {
 $epoch=floor(($ticks - 621355968000000000) / 10000000); 
