...
if(!empty($_SERVER['HTTP_HOST'])){
    ...
    if($domain == 'touchtop.babaerp.live'){
        // dd(auth()->user()->getAllPermissions());
            ...
            //Products dropdown
            if (in_array('products', $enabled_modules) && (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\PriceGroupController::class, 'updateProductPrice']),
                        ...
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\PriceGroupController::class, 'index']),
                                __('lang_v1.price_group'),
                                ['icon' => 'fa fas fa-circle', 'active' => request()->segment(1) == 'price-group']
                            );
                        }
            ...
            if (($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return']))) {
             ...
             if (in_array('add_sale', $enabled_modules) ) {
                        ...
                        if (auth()->user()->can('sell.view')) {
                            $sub->url(
                                action([SellPosController::class, 'getVoidProduct']),
                                __('sale.voided_product'),
                                ['icon' => 'fa fas fa-list', 'active' =>  request()->segment(2) == 'get-void-product']
                            );
                        }
                    },
            ...
            if (in_array('stock_transfers', $enabled_modules) &&
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.create') || auth()->user()->can('stock_transfer.view')) {
                            $sub->url(
                        ...
                        if (auth()->user()->can('purchase.create') || auth()->user()->can('stock_transfer.create')) {
                            $sub->url(
            ...
            //Stock Taking
            if (in_array('stock_taking', $enabled_modules) && auth()->user()->can('product.stock_taking')) {
                $menu->url(action([StockTakingController::class, 'getImportInventory']), __('lang_v1.stock_taking'),
                    ['icon' => 'fa fas fa-database', 'active' => request()->segment(1) == 'stock-taking'])->order(40);
            }
            ...
            //Reports dropdown
            if (!auth()->user()->can("superadmin") || auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                             if (in_array('products', $enabled_modules)){
                                 $sub->url(
                                    action([\App\Http\Controllers\ReportController::class, 'getStockTransferReport']),
                                    __('report.stock_transfer'),
                                    ['icon' => 'fa fas fa-clock', 'active' => request()->segment(2) == 'stock-transfer-report']
                                );
                            $sub->url(
...
if (in_array('add_sale', $enabled_modules)){
            ...
            if (auth()->user()->can('superadmin') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules,$common_settings, $is_admin) {
                        ...
                        if (!empty($common_settings['enable_multi_currency']) && auth()->user()->can('access_multi_currency_settings')) {
                            $sub->url(
                                action([MultiCurrencyController::class, 'getSettings']),
                                __('lang_v1.multi_currency_settings'),
                                ['icon' => 'fa fas fa-plus-circle', 'active' => request()->segment(1) == 'multi-currency-settings']
                            );
                        }
                        ...
                        //Permission should be set here
                        // if(auth()->user()->can('superadmin')) {
                        //     $sub->url(
                        //         action([\App\Http\Controllers\SubdomainController::class, 'getSubDomains']),
                        //         __('subdomain.babaerp_subdomain'),
                        //         ['icon' => 'fa fas fa-user-circle', 'active' => request()->segment(1) == 'registered-subdomains']
                        //     );
                        // }
                    },
