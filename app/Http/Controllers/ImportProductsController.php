                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        if (! empty($value[21]) && $enable_stock == 1) {
                            ...
                            //Stock expiry date
                            if (! empty($value[23])) {
                                $product_array['opening_stock_details']['exp_date'] = \Carbon::createFromFormat('d-m-Y', trim($value[23]))->format('Y-m-d');
                            } else {
    ...
    private function calculateVariationPrices($dpp_exc_tax, $dpp_inc_tax, $selling_price, $tax_amount, $tax_type, $margin)
    {
        ...
        if ($selling_price != 0) {
            // $selling_price = (float) $selling_price;
                // dd($dpp_inc_tax, $dpp_exc_tax,$selling_price);
            ...
            // dd( $this->productUtil->num_f($dpp_exc_tax),  $this->productUtil->num_f($dpp_inc_tax), $this->productUtil->num_f($dsp_inc_tax));
