...
use App\Action\BusinessCustomLabelsAction;
use App\Action\CalculateDebitPaidByCashAction;
...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        $selling_price_groups = PriceGroup::where('business_id', $business_id)
                                                ->get();
        $allowed_selling_price_group = false;
        foreach ($selling_price_groups as $selling_price_group) {
            if (auth()->user()->can('price_group.'.$selling_price_group->id)) {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
                'only_mfg_products', 'active_state',  'not_for_selling', 'repair_model_id', 'product_id', 'active_state', 'stock_status_id','start_date', 'end_date']);
            ...
            $datatable = Datatables::of($products)
                ...
                ->editColumn('opening_stock_as_of', function ($row) {
                    if ($row->enable_stock) {
                        $stock = $row->opening_stock_as_of ? $row->opening_stock_as_of : 0;
                 ...
                 ->editColumn('closing_stock_as_of', function ($row) {
                    if ($row->enable_stock) {
                        $stock = $row->closing_stock_as_of ? $row->closing_stock_as_of : 0;
                ...
                ->addColumn('variation', function ($row) {
                  ...
                  ->editColumn('total_purchase', function ($row) {
                    ...
                    return '<span class="total_transfered" data-orig-value="'.$total_purchase.'" data-unit="'.$row->unit.'" >'.$this->transactionUtil->num_f($total_purchase, false, null, true).'</span> '.$row->unit;
                })
                ...
                ->editColumn('purchase_price', function ($row) {
                     $purchase_price = (float) $row->purchase_group_price > 0 ? $row->purchase_group_price : $row->purchase_price;

                    return $this->transactionUtil->num_f($purchase_price, true);
                    
                })
                ->editColumn('unit_price', function ($row) use ($allowed_selling_price_group) {
                    $html = '';
                    $unit_selling_price = (float) $row->selling_group_price > 0 ? $row->selling_group_price : $row->unit_price;

                    if (auth()->user()->can('access_default_selling_price')) {
                        $html .= $this->transactionUtil->num_f($unit_selling_price, true);
                    }
                ...
                ->editColumn('stock_price', function ($row) {
                    ...
                    $stock_price = $purchase_price * $stock;

                   return '<span class="total_stock_price" data-orig-value="'
                        .$stock_price.'">'.
                ...
                ->addColumn('potential_profit', function ($row) {
                    ...
                    $stock_price_by_pp = $stock * $row->purchase_price;
//                    $potential_profit = (float) $stock_price_by_sp - (float) $row->stock_price;

                    $potential_profit = (float) $stock_price_by_sp - (float) $stock_price_by_pp;
            ...
            $raw_columns = ['unit_price', 'purchase_price','total_transfered', 'total_sold',
                'total_adjusted', 'stock', 'stock_price', 'stock_value_by_sale_price', 'total_purchase',
                'potential_profit', 'action', 'opening_stock_as_of','closing_stock_as_of'];
    ...
    public function getTaxReport(Request $request)
    {
        ...
        public function printTrendingProducts(Request $request)
    {
        ...
        $filters = $request->only([
            'category', 'sub_category', 'brand', 'unit', 'limit',
            'location_id', 'product_type', 'less_trending_products', 'date_range'
        ]);
        


        // Date parsing (same as in getTrendingProducts)
        if (!empty($filters['date_range'])) {
            [$from, $to] = array_map('trim', explode('~', $filters['date_range']));
            $filters['start_date'] = $this->transactionUtil->uf_date($from);
            $filters['end_date']   = $this->transactionUtil->uf_date($to);
        }

        $limit = (int)($filters['limit'] ?? 5);

        // Query the same data source you already use
        $products = $this->productUtil->getTrendingProducts($business_id, $filters);

        $business = Business::findOrFail($business_id);

        // Build rows for PDF
        $rows = collect($products)->take($limit)->map(function ($p) {
            return [
                'name'    => trim(($p->product ?? '') . ' - ' . ($p->sku ?? '') . (!empty($p->unit) ? ' ('.$p->unit.')' : '')),
                'units'   => (float)($p->total_unit_sold ?? 0),
                // Use your actual revenue field if you have it; keep 0.0 if not present
                'revenue' => (float)($p->total_revenue ?? 0),
            ];
        });

        // Metadata (kept lightweight; you can expand with lookups if you want full names)
        $meta = [
            'business' => $business,
            'title'       => $request->boolean('less_trending_products') ? __('Less Trending Products') : __('report.top_trending_products'),
            'date_range'  => $filters['date_range'] ?? __('messages.all'),
            'limit'       => $limit,
            'generated_at'=> now()->format('Y-m-d H:i'),
            'applied'     => array_filter([
                'location_id'   => $filters['location_id'] ?? null,
                'category'      => $filters['category'] ?? null,
                'sub_category'  => $filters['sub_category'] ?? null,
                'brand'         => $filters['brand'] ?? null,
                'unit'          => $filters['unit'] ?? null,
                'product_type'  => $filters['product_type'] ?? null,
                'less_trending' => $request->boolean('less_trending_products') ? '1' : null,
            ]),
        ];

        // If exactly 5 rows, use the “branded sheet” layout; otherwise generic table
        $view = 'report.pdf.trending_products';
        ...
        // stream in browser
        return $pdf->stream('trending_products.pdf');
    }
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        $custom_labels = (new BusinessCustomLabelsAction($this->businessUtil))->handle(business_id: $business_id, label_type: LabelType::PAYMENT);
        ...
        if ($request->ajax()) {
            $registers = CashRegister::leftjoin(
                ...
                ->leftJoin('transactions', 'transactions.id', '=', 'ct.transaction_id')
                ->where('cash_registers.business_id', $business_id)
                ->where('ct.deleted_at', null)
                ->select(
                    'ct.pay_method',
                    'cash_registers.*',
                    'ct.transaction_id as ct_tid',
                    'ct.created_at as ct_time',
                    DB::raw(
                    ...
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='cheque' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0, amount, 0)) FROM transaction_payments WHERE transaction_payments.transaction_id = ct.transaction_id  HAVING COUNT(DISTINCT method) > 1), 0)) as total_cheque_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='cash' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0, amount, 0)) FROM transaction_payments WHERE transaction_payments.transaction_id = ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_cash_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='card' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id  HAVING COUNT(DISTINCT method) > 1), 0)) as total_card_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='bank_transfer' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_bank_transfer_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='advance' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_advance_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_1' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_1_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_2' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_2_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_3' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_3_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_4' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_4_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_5' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_5_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_6' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_6_refund"),
                    DB::raw("SUM(IF(transaction_type='refund' AND pay_method='custom_pay_7' AND ct.type = 'debit',(SELECT SUM(IF(is_return = 0,amount,0)) FROM transaction_payments WHERE transaction_payments.transaction_id=ct.transaction_id HAVING COUNT(DISTINCT method) > 1), 0)) as total_custom_pay_7_refund"),
                )->groupBy('cash_registers.id');

            //Debits
            $debits = DB::table('account_transactions as at')
                ...
                ->whereIn('t.type', ['expense', 'purchase'])
                ->where([
                ...
                ->select(DB::raw("SUM(at.amount) as debit"));
                
                // dd($debits->toBoundSql());

            //Cash Refund if any
            $cash_refund_change = DB::table('cash_register_transactions as CT')
                ->rightJoin('cash_registers as CR','CR.id', 'CT.cash_register_id')
                ->join('transaction_payments as TP', 'TP.transaction_id', 'CT.transaction_id')
                ->where([
                    'TP.business_id' => $business_id,
                    'is_return' => 1,
                    'transaction_type' => 'refund',
                    'pay_method' => 'cash'
                ])->select(DB::raw("SUM(TP.amount) as cash_refund_change"));
            ...
            if ($permitted_locations != 'all') {
                ...
                $debits->whereIn('t.location_id', $permitted_locations);
                $cash_refund_change->whereIn('CR.location_id', $permitted_locations);
                                $filters['permitted_location'] = $permitted_locations;

            }

            if (! empty($request->input('user_id'))) {
                $registers->where('cash_registers.user_id', $request->input('user_id'));
                $cash_refund_change->where('CR.user_id', $request->input('user_id'));
            }
            ...
            if (! empty($start_date) && ! empty($end_date)) {
                ...
                $debits->whereDate('operation_date', '>=', $start_date)
                    ->whereDate('operation_date', '<=', $end_date);
                    

                $cash_refund_change->whereDate('CR.created_at', '>=', $start_date)
                    ->whereDate('CR.created_at', '<=', $end_date);
                    
                     $filters['start_date'] = $start_date;
                $filters['end_date'] = $end_date;

                //Get return products
                $returned_t = Transaction::query()
                    ->where([
                        'type' => 'sell_return',
                        'payment_status' => 'paid'
                    ])
                    ...
                    ->whereIn('return_parent_id', function ($q){
                        $q->select(\DB::raw('id'))
                            ->from('transactions')
                ...
                if (!empty($request->get('business_location'))) {
                    $returned_t->where('transactions.location_id', $request->get('business_location'));
                };

                $returned_get = $returned_t->get([
                    'total_before_tax',
                    'tax_amount'
                ]);
                if (!empty($returned_get) && 0 < count($returned_get)) {
                    foreach ($returned_get as $returned) {
                        $total_returned += $returned->total_before_tax + $returned->tax_amount;
                    }
                }
            }
            if (!empty($request->get('business_location'))) {
                $location = $request->get('business_location');
                $loc_name = BusinessLocation::select('name')->where('id', $location)->first(['name'])['name'];

                $registers->where('cash_registers.location_id', $location);
                $debits->where('t.location_id', $location)
                    ->where('accounts.name', 'LIKE', "%$loc_name%");

                $cash_refund_change->where('CR.location_id', $location);
            ...
            // dd($debits);
            $debit = 0;
            // if (!is_null($debits[0]->debit)) {
            //     $debit = $debits[0]->debit;
            // }
            
              //Total Debit (Expenses + Purchases)
            $debit_calculated = (new CalculateDebitPaidByCashAction($filters))->handle();
            $debit = $debit_calculated->total_debit ?: 0;
            // dd($debit);

            //Cash Change
            $cash_refund_change = $cash_refund_change->get();
            // dd($cash_refund_change);
            $cash_change = 0;
            if (!is_null($cash_refund_change[0]->cash_refund_change)) {
                $cash_change = $cash_refund_change[0]->cash_refund_change;
            }
            ...
            return Datatables::of($registers)
                ...
                ->editColumn('total_cash_refund', function ($row) {
                    return '<span data-orig-value="' . $row->total_cash_refund . '" >' . $this->transactionUtil->num_f($row->total_cash_refund, true) . '</span>';
                })
                ...
                ->addColumn('total', function ($row) {
                 ...
                 ->editColumn('total_returned', function ($row) use($total_returned) {
                    return '<span data-orig-value="' . $total_returned . '" >' . $this->transactionUtil->num_f($total_returned, true) . '</span>';
                })
                 ->editColumn('debit', function ($row) use ($debit) {
                    return '<span data-orig-value="' . $debit . '" >' . $this->transactionUtil->num_f($debit, true) . '</span>';
                })
                 ->editColumn('cash_refund_change', function () use($cash_change) {
                    return '<span data-orig-value="' . $cash_change . '" >' . $this->transactionUtil->num_f($cash_change, true) . '</span>';
                })
                ...
                ->rawColumns(['total_returned', 'debit', 'action', 'user_name', 'total_card_payment', 'total_cheque_payment', 'total_cash_payment', 'total_bank_transfer_payment', 'total_other_payment', 'total_advance_payment', 'total_custom_pay_1', 'total_custom_pay_2', 'total_custom_pay_3', 'total_custom_pay_4', 'total_custom_pay_5', 'total_custom_pay_6', 'total_custom_pay_7', 'total',
                    'total_cash_refund',
                    'total_cheque_refund',
                    'total_card_refund',
        ...
        return view('report.register_report')
                    ->with(compact('users', 'custom_labels','business_locations'));
    }
    ...
    public function activityLog()
    {
       ...
       $transaction_types = [
           'contact' => __('report.contact'),
            ...
            'price_group' => __('lang_v1.price_group'),
            ...
            'stock_taking' => __('lang_v1.stock_taking'),
            'user' => __('report.user'),
        ];
        if (request()->ajax()) {
            ...
            if (! empty($subject_type)) {
                               match ($subject_type) {
                    'contact' => $activities->where('subject_type', Contact::class),
                    'user' => $activities->where('subject_type', User::class),
                    'product' => $activities->where('subject_type', Product::class)
                        ->whereIn('description', ['added', 'edited', 'deleted']),
                    'price_group' => $activities->where('subject_type', VariationGroupPrice::class),
                    'stock_taking' => $activities->where('subject_type', 'App\VariationLocationDetails'),
                    'opening_stock' => $activities->where('description', $subject_type)
                        ->whereIn('subject_type', [Product::class, \App\Variation::class]),

                    'sell', 'purchase','stock_adjustment',
                    'sales_order', 'purchase_order',
                    'sell_return', 'purchase_return',
                    'sell_transfer', 'expense' => $activities->where('subject_type', \App\Transaction::class)
                        ->whereHasMorph('subject', Transaction::class, function ($q) use ($subject_type) {
            ...
            return Datatables::of($activities)
                            ...
                            ->addColumn('subject_type', function ($row) use ($transaction_types) {
                               if ($row->subject_type == \App\Transaction::class && !empty($row->subject->type)) {
                                    return $transaction_types[$row->subject->type] ?? '';
                                }
                                return match ($row->subject_type) {
                                    Contact::class => __('contact.contact'),
                                    Product::class, \App\Variation::class => __('lang_v1.product'),
                                    VariationGroupPrice::class => __('lang_v1.price_group'),
                                    TransactionPayment::class => __('lang_v1.payment'),
                                    User::class => __('report.user'),
                                    VariationLocationDetails::class => __('lang_v1.stock_taking'),
                                    default => ''
                            ...
                            ->addColumn('note', function ($row) use ($statuses, $shipping_statuses) {
                                ...
                                if ($row->subject_type == 'App\Product'
                                    || $row->subject_type == 'App\Variation'
                                    || $row->subject_type == 'App\VariationLocationDetails'
                                    || $row->subject_type == 'App\VariationGroupPrice') {
                                    $html .= view('sale_pos.partials.activity_row', ['activity' => $row])->render();
                                }
    ...
    /**
     * Shows stock evaluation report
     * @param Request $request
     */
    public function getStockEvaluationByDepartmentReport(Request $request)
    {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        $location = session('location_id');
        $filters['location_id'] = !empty($location) && $location != 'none' ? $location : null;
        $business = $request->session()->get('business')['name'];

        $location_name = !empty($location) && $location != 'none' ? strtoupper(BusinessLocation::getLocationName($location)) : "ALL LOCATIONS";
        $business_id = $request->session()->get('user.business_id');

        $report = $this->productUtil->getEvaluationByDepartment($business_id, $filters);
        return view('product.stock_evaluation_report')->with(compact('business', 'location_name','report'));

    }
         public function getSellsSummaryReport(Request $request)
     {
        if (!auth()->user()->can('stock_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $location_id = !empty($location_id) && $location_id != 'none' ? $location_id : null;;
        // dd($location_id);

        $dates = [
            'start' => !empty($start_date) ? \Carbon::createFromTimestamp(strtotime($start_date))->format('d F Y') : null,
            'end' => !empty($end_date) ? \Carbon::createFromTimestamp(strtotime($end_date))->format('d F Y') : null
        ];

        $business = $request->session()->get('business')['name'];
        $business_id = $request->session()->get('user.business_id');

        $location_name = !empty($location_id) && $location_id != 'none' ? strtoupper(BusinessLocation::getLocationName($location_id)) : "ALL LOCATIONS";

         $taxes = TaxRate::forBusiness($business_id);
         $amount = !empty($taxes) ? $taxes[0]['amount'] :0;
         ...
         $output['total_sell_return'] = ! empty($transaction_totals['total_sell_return_inc_tax']) ? $transaction_totals['total_sell_return_inc_tax'] : 0;
         $output['total_sell_return_paid'] = $this->transactionUtil->getTotalSellReturnPaid($business_id, $start_date, $end_date, $location_id);

         $output['total_sell'] = ! empty($sell_details['total_sell_inc_tax']) ? $sell_details['total_sell_inc_tax'] : 0;
         $output['total_sell_exc_tax'] = ! empty($sell_details['total_sell_exc_tax']) ? $sell_details['total_sell_exc_tax'] : 0;


         $output['total_expense'] = $transaction_totals['total_expense'];
         $output['total_expense_paid'] = $this->transactionUtil->getTotalExpensePaid($business_id, $start_date, $end_date, $location_id);

         $output['invoice_due'] = $sell_details['invoice_due'];

         $output['net_sell'] = $output['total_sell'] - $output['invoice_due'];


        //  $payment_query = TransactionPayment::query()
        //      ->join('transactions as t', function ($join){
        //          $join->on('transaction_payments.transaction_id',
        //              '=', 't.id')->whereNull('t.deleted_at');
        //      })
        //      ->where([
        //          't.business_id' => $business_id,
        //          't.type' => TransactionType::SELL,
        //          't.status' => TransactionStatus::FINALLIZED,
        //      ]);


        //  $purchases_query = TransactionPayment::query()
        //      ->join('transactions as t', function ($join){
        //          $join->on('transaction_payments.transaction_id',
        //              '=', 't.id')->whereNull('t.deleted_at');
        //      })
        ...
        //  if (!empty($start_date) && !empty($end_date)) {
        //      $payment_query->where('t.transaction_date', '>=', $start_date)
        //          ->where('t.transaction_date', '<=', $end_date);

        //      $purchases_query->where('transaction_payments.paid_on', '>=', $start_date)
        //          ->where('transaction_payments.paid_on', '<=', $end_date);
        //  }
        //  if (!empty($location_id)) {
        //      $payment_query->where('t.location_id', $location_id);
        //      $purchases_query->where('t.location_id', $location_id);
        //  }

        //  $output['sells_payments'] = $payment_query->select('method', \Illuminate\Support\Facades\DB::raw('SUM(IF(is_return = 0, amount, amount*-1)) as sell_payment'))->groupBy('method')->get();
        //  $output['purchase_payments'] = $purchases_query->select('method', \Illuminate\Support\Facades\DB::raw('SUM(amount) as purchases_payment'))->groupBy('method')->get();
        //  $output['payment_types'] = $this->transactionUtil->payment_types(null, true, $business_id);
         //Sort using key
         //rsort($payment_types);
         ...
         $payment_query = clone $query1->where([
             't.type' => TransactionType::SELL,
             't.status' => TransactionStatus::FINALLIZED,
         ]);

         $purchases_query = clone $query2->where([
             't.type' => TransactionType::PURCHASE,
         ]);
       


         //Clone previous query to be used
         $due_invoice_payment_query = clone $payment_query;


         if (!empty($start_date) && !empty($end_date)) {
             $payment_query->where('transaction_payments.paid_on', '>=', $start_date)
                 ->where('transaction_payments.paid_on', '<=', $end_date);
             ...
             //Other payments
             $due_invoice_payment_query
                 ->where('transaction_payments.paid_on', '>=', $start_date)
                 ->where('transaction_payments.paid_on', '<=', $end_date)
                 ->where('t.transaction_date', '<', $start_date)
                 ->whereColumn('transaction_payments.paid_on', '>', 't.transaction_date');
         }

         if (!empty($location_id)) {
             $due_invoice_payment_query->where('t.location_id', $location_id);
             $payment_query->where('t.location_id', $location_id);
             $purchases_query->where('t.location_id', $location_id);
         }
         ...
         $output['due_invoice_payment']= $due_invoice_payment_query->select('method', 'paid_on','amount','t.transaction_date as t_date', 't.invoice_no as invoice')
             ->get()
             ->groupBy(function ($val){
                 return \Carbon::parse($val->paid_on)->format('d-M-y');
             })->toArray();
...
//        asort($payment_types);
         ...
         return view('sell.sells_summary_report', compact('business', 'location_name', 'dates', 'output'));
    }
        public function getStockTransferReport(Request $request)
    {
        ...
        $from_business_locations = BusinessLocation::forDropdown($business_id, true);
        $from_business_locations->prepend(__('report.all_locations'), 'all');

        $to_business_locations = BusinessLocation::forTransferToDropdown($business_id);
        ...
        if ($request->ajax()) {
            ...
            $query = Transaction::query()
                ...
                ->join('variations as VR','VR.id', 'SL.variation_id')
                ->join('products', 'products.id', 'VR.product_id')
                ->join('business_locations AS l1',
                    'transactions.location_id',
                    '=',
                    'l1.id'
                )
                ...
                ->where('transactions.status', 'final')
            ->whereIn('transactions.type', ['sell_transfer', 'purchase_transfer']);

            $stock_transfers = $query
                ->select(
                    'transactions.id',
                    'transactions.type',
                    'transactions.transaction_date',
                    'transactions.ref_no',
                    DB::raw('unit_price_inc_tax * quantity as final_total'),
            ...
            if (request()->has('to_location_id') || request()->has('from_location_id')) {
                $from_location = request()->get('from_location_id');
                $to_location = request()->get('to_location_id');

                if (!empty($from_location)) {
                    if ($from_location == 'all' && $permitted_locations != 'all') {
                        $stock_transfers->where(function ($query) use ($permitted_locations) {
                            $query->whereIn('l2.id', $permitted_locations)->orwhereIn('l1.id', $permitted_locations);
                        });
                    } else if ($from_location != 'all') {
                        if ($permitted_locations == 'all') {
                            $stock_transfers->where('l1.id', $from_location);
                        } else {
                            $stock_transfers->where('l1.id', $from_location);
                        }
                    }
                ...
                if (!empty($to_location)) {
                    $stock_transfers->where('t2.location_id', $to_location);
                }
            }

            return \Yajra\DataTables\Facades\DataTables::of($stock_transfers)
                ->addColumn('unit_price',
        ...
        return view('report.stock_transfer_report')
            ->with(compact(  'from_business_locations','to_business_locations'));
