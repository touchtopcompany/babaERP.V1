...
class Util
{
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        ...
        $name  = Str::lower($contact_payments->contact_name);
        if ($contact_payments->contact_type == 'customer' && str_contains($name, 'walk')) {
            $due = 0;
        }
        return $due;
    }
    ...
    public function calculateProductMultiplier($business_id, $product_or_sell_line, $value, $is_product = false)
    {
        $multiplier = 1;
        $product_labels = (new BusinessCustomLabelsAction($this))->handle($business_id, LabelType::PRODUCT);
        if (!empty($product_labels)) {
            $custom_field_value = !$is_product
                // is it from sell line
                ? $product_or_sell_line->product->product_custom_field2
                //is it from product
                : $product_or_sell_line->product_custom_field2;
            foreach ($product_labels as $label) {
                if (Str::contains($label, $value['name']) && !empty($custom_field_value)) {
                    $multiplier = (int)$custom_field_value;
                    break;
                }
            }
        }
