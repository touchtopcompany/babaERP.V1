...
class ProductUtil extends Util
{
     ...
     */
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true, $nullable_product = false)
    {
        ...
        $product = $query->select(
         ...
         if(!is_null($location_id))
             $product->addSelect( DB::raw("(SELECT purchase_price_inc_tax FROM variation_group_prices AS VGP JOIN business_locations AS BL ON BL.selling_price_group_id = VGP.price_group_id WHERE
                     variation_id=$variation_id AND BL.id = $location_id LIMIT 1) as group_purchasing_price"));

         if ($nullable_product) return $product->first();
    ...
    public function getTrendingProducts($business_id, $filters = [])
    {
        ...
        $products = $query->select(
            DB::raw('(SUM(tsl.quantity) - COALESCE(SUM(tsl.quantity_returned), 0)) as total_unit_sold'),
                        DB::raw('SUM( (tsl.quantity - COALESCE(tsl.quantity_returned,0)) * tsl.unit_price_inc_tax ) AS total_revenue'),
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            foreach ($input['products'] as $product) {
                if (! empty($product['transaction_sell_lines_id'])) {
                    $this->updateProductQuantity($input['location_id'], $product['product_id'], $product['variation_id'], !empty($product['base_unit_multiplier']) ? $product['base_unit_multiplier'] * $product['quantity'] : $product['quantity'], 0, null, false);
            ...
            foreach ($input['products'] as $product) {
                ...
                if (!empty($product['base_unit_multiplier'])) {
                    $uf_quantity = $this->num_uf($uf_quantity * $product['base_unit_multiplier']);
                }
    ...
    {
        //Check to see if the location has been set with price group
        $update_default_price = false;
        $is_group_price_exists = false;
        $group_price_id = BusinessLocation::query()->findOrFail($variation_data['location_id'])->selling_price_group_id;

        //Check for existing variation group pricing
        $data = [
            'variation_id' => (int)$variation_data['variation_id'],
            'price_group_id' => $group_price_id
        ];

        if (!is_null($group_price_id) && ($group_price_id > 0)) {

            $query = VariationGroupPrice::query()
                ->where($data);

            if ($query->exists()) {
                $variation_details = $query->first();
                $is_group_price_exists = true;
            } else {
               VariationGroupPrice::query()
                ->create(
                    $data + [
                        'sell_price_inc_tax' => $variation_data['sell_price_inc_tax'],
                        'purchase_price_inc_tax' => $variation_data['purchase_price_inc_tax'],
                    ]
                );
            }
        }
        else{
            $update_default_price = true;
            $variation_details = Variation::query()->where('variations.id', $variation_data['variation_id'])
                ->with(['product', 'product.product_tax'])->first();
        }

        //We're able to fetch product information
        if ($is_group_price_exists || $update_default_price) {
            $tax_rate = 0;
           ...
           if ((isset($variation_details->default_purchase_price) && $variation_details->default_purchase_price != $variation_data['pp_without_discount']) ||
                (isset($variation_details->sell_price_inc_tax) && $variation_details->sell_price_inc_tax != $variation_data['sell_price_inc_tax'])
            ) {
                if ($update_default_price) {
                    ...
                    $variation_details->dpp_inc_tax = $this->calc_percentage_base($variation_details->default_purchase_price, $tax_rate, $variation_details->default_purchase_price);
                    ...
                    //Update this location group price
                    VariationGroupPrice::query()
                        ->where([
                            'variation_id' => $variation_details->variation_id,
                            'price_group_id' => $variation_details->price_group_id
                        ])->update([
                            'sell_price_inc_tax' => $this->calc_percentage_base($variation_data['sell_price_inc_tax'],$tax_rate, $variation_data['sell_price_inc_tax']),
                            'purchase_price_inc_tax' => $this->calc_percentage_base($variation_data['purchase_price_inc_tax'], $tax_rate, $variation_data['purchase_price_inc_tax'])
                        ]);
                }
            }
        }
    ...
    public function getVariationGroupPrice($variation_id, $price_group_id, $tax_id)
    {
        $price_group = VariationGroupPrice::where('variation_id', $variation_id)
                        ...
                        ->select(['sell_price_inc_tax', 'price_type'])
                        ->first();
        ...
        if(isset($price_group)) {
            if ($price_group->price_type == 'percentage') {
                //calculate the price
                $variation = Variation::find($variation_id);
                $price_inc_tax = $this->calc_percentage($variation->sell_price_inc_tax, $price_group->sell_price_inc_tax);
            } else {
                $price_inc_tax = $price_group->sell_price_inc_tax;
            }
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            if ($enable_product_editing == 1 && $transaction->type == 'purchase') {
                ...
                $variation_data['default_purchase_price'] = $purchase_line->purchase_price;
                $variation_data['purchase_price_inc_tax'] = $purchase_line->purchase_price_inc_tax;
                $variation_data['location_id'] = $transaction->location_id;
    ...
    public function changeSellLineUnit($business_id, $sell_line)
    {
        ...
        if (! empty($sub_unit)) {
            ...
            $calculate_multiplier = $this->calculateProductMultiplier($business_id, $sell_line, $sub_unit);
            if($calculate_multiplier != 1)
            {
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        if (! empty($price_group_id)) {
            $query->addSelect(DB::raw('IF (VGP.price_type = "fixed", VGP.sell_price_inc_tax, VGP.sell_price_inc_tax * variations.sell_price_inc_tax / 100) as variation_group_price'));
            $query->addSelect(DB::raw('IF (VGP.price_type = "fixed", VGP.purchase_price_inc_tax, VGP.purchase_price_inc_tax * variations.dpp_inc_tax / 100) as purchase_group_price'));
        }
    ...
    public function getProductStockDetails_orginal($business_id, $filters, $for)
    {
        ...
        // Build once; reuse across subqueries. Uses DATE() to be safe against time portions.
        $date_filter = '';
        if ($start && $end) {
            $date_filter = " AND transactions.transaction_date >= '{$start->toDateTimeString()}' AND transactions.transaction_date < '{$end->toDateTimeString()}'";
        } elseif ($start) {
            $date_filter = " AND transactions.transaction_date >= '{$start->toDateTimeString()}'";
        } elseif ($end) {
            $date_filter = " AND transactions.transaction_date < '{$end->toDateTimeString()}'";
        }
        ...
        if ($for == 'view_product' && ! empty(request()->input('product_id'))) {
         ...
         $products = $query->select(
            ...
            DB::raw("(SELECT SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ) FROM transactions 
                  JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                  WHERE transactions.status='final' AND transactions.type='sell_transfer' AND transactions.location_id=vld.location_id {$date_filter} AND (TSL.variation_id=variations.id)) as total_transfered"),
            DB::raw("(SELECT SUM(IF(transactions.type='stock_adjustment', SAL.quantity, 0) ) FROM transactions 
                  JOIN stock_adjustment_lines AS SAL ON transactions.id=SAL.transaction_id
                  WHERE transactions.type='stock_adjustment' AND transactions.location_id=vld.location_id {$date_filter}
            ...
            DB::raw("(SELECT SUM( COALESCE(pl.quantity - ($pl_query_string), 0) * purchase_price_inc_tax) FROM transactions 
                  ...
                  WHERE (transactions.status='received' OR transactions.type='purchase_return')  AND transactions.location_id=vld.location_id {$date_filter}
            ...
            'p.product_custom_field3',
            'p.product_custom_field4'
        )->groupBy('variations.id', 'vld.location_id')->orderBy('c.name')->orderBy('p.name');

 if ($end) {
            $endString = $filters['end_date'];
        ...
        (
          COALESCE((
            SELECT SUM(
              CASE
                WHEN transactions.status='received'      THEN pl.quantity
                WHEN transactions.type='purchase_return' THEN -pl.quantity
                ELSE 0
              END
            )
            FROM transactions
            JOIN purchase_lines AS pl ON transactions.id = pl.transaction_id
            WHERE transactions.location_id = vld.location_id
        ...
        ) AS closing_stock_as_of
    ", [$endString, $endString, $endString, $endString]);
        }


        // Optional: Opening stock before $start
        if ($start) {
            $startString = $filters['start_date'];
    ...
    ) AS opening_stock_as_of
    ", [$startString, $startString, $startString, $startString]);
        }
        
        //  $_start = $filters['start_date'] ?? null;
        // $_end = $filters['end_date'] ?? null;

//  $product_query = $query->select(
//             // DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $location_filter AND
//             //     transaction_sell_lines.product_id=products.id) as total_sold"),
//             DB::raw('SUM(vld.qty_available) as stock'),
//             'variations.sub_sku as sku',
...
//         }
        ...
        // $products = $product_query->groupBy('variations.id', 'vld.location_id')->orderBy('c.name')->orderBy('p.name');
        ...
        if (!empty($filters['location_id'])) {
            $products->addSelect(DB::raw("(SELECT purchase_price_inc_tax FROM variation_group_prices AS VGP JOIN business_locations AS BL ON BL.selling_price_group_id = VGP.price_group_id WHERE variations.id = VGP.variation_id AND BL.id = vld.location_id) AS purchase_group_price"));
            $products->addSelect(DB::raw("(SELECT sell_price_inc_tax FROM variation_group_prices AS VGP JOIN business_locations AS BL ON BL.selling_price_group_id = VGP.price_group_id WHERE variations.id = VGP.variation_id AND BL.id = vld.location_id) AS selling_group_price"));
        }

        if (!empty($filters['stock_status_id'])) {
            $stock_status = $filters['stock_status_id'];

            if ($stock_status == StockStatus::POS_NEG) {
                $query->havingRaw("SUM(vld.qty_available) < 0 OR SUM(vld.qty_available) > 0");
            }
            else {
                $sum_condition = match ($stock_status) {
                    StockStatus::NEGATIVE => '<',
                    StockStatus::POSITIVE => '>',
                    StockStatus::ZERO => '=',
                    default => throw new \Exception("Undefined stock status"),
                };
                $query->havingRaw("SUM(vld.qty_available) $sum_condition 0");
            }
        }
        ...
        if ($for == 'view_product') {
            return $products->orderBy('l.name')->get();
        } elseif ($for == 'api') {
    ...
    public function getProductStockDetails_bk($business_id, $filters, $for)
    {
        $start = !empty($filters['start_date']) ? \Carbon::parse($filters['start_date']) : null;
        $end   = !empty($filters['end_date'])   ? \Carbon::parse($filters['end_date'])   : null;
        // dd($start, $end);
        ...
        $permitted_locations = auth()->user()->permitted_locations();
        $restrict_locations  = is_array($permitted_locations) && $permitted_locations !== 'all';

        // ---------- Base rows: (variation_id x location_id) ----------
        $base = DB::table('variations')
        ...
        if ($restrict_locations) {
            $base->whereIn('vld.location_id', $permitted_locations);
        }
        if (!empty($filters['location_id'])) {
            $locId = (int) $filters['location_id'];
            // persist for stock report as per your original
            session(['location_id' => $locId]);

            $base->where('vld.location_id', $locId)
                ->join('product_locations as ploc', function ($j) use ($locId) {
                    $j->on('ploc.product_id', '=', 'p.id')
                        ->where('ploc.location_id', '=', $locId);
                });
        }
        if (!empty($filters['category_id'])) {
            $cat = $filters['category_id'];
            if (is_array($cat)) {
                $cat[0] === 'none' ? $base->whereNull('p.category_id') : $base->whereIn('p.category_id', $cat);
            } else {
                $cat === 'none' ? $base->whereNull('p.category_id') : $base->where('p.category_id', $cat);
            }
        }
        if (!empty($filters['sub_category_id'])) $base->where('p.sub_category_id', $filters['sub_category_id']);
        if (!empty($filters['brand_id']))       $base->where('p.brand_id', $filters['brand_id']);
        if (!empty($filters['unit_id']))        $base->where('p.unit_id', $filters['unit_id']);
        if (!empty($filters['tax_id']))         $base->where('p.tax', $filters['tax_id']);
        if (!empty($filters['type']))           $base->where('p.type', $filters['type']);

        if (isset($filters['only_mfg_products']) && $filters['only_mfg_products'] == 1) {
            $base->join('mfg_recipes as mr', 'mr.variation_id', '=', 'variations.id');
        }
        if (isset($filters['active_state'])) {
            $base->where('p.is_inactive', $filters['active_state'] === 'inactive' ? 1 : 0);
        }
        if (isset($filters['not_for_selling']) && $filters['not_for_selling'] == 1) {
            $base->where('p.not_for_selling', 1);
        }
        if (!empty($filters['repair_model_id'])) {
            $base->where('p.repair_model_id', $filters['repair_model_id']);
        }

        // ---------- Window aggregates (sold / transferred / adjusted) ----------
        $txWindow = DB::table('transactions as t')
            ->selectRaw("
            COALESCE(tsl.variation_id, sal.variation_id) as variation_id,
            t.location_id,
            SUM(CASE WHEN t.status='final' AND t.type='sell'          THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS sold_qty,
            SUM(CASE WHEN t.status='final' AND t.type='sell_transfer' THEN  tsl.quantity                         ELSE 0 END) AS transferred_qty,
            SUM(CASE WHEN t.type='stock_adjustment'                   THEN  sal.quantity                         ELSE 0 END) AS adjusted_qty
        ")
            ...
            ->when($restrict_locations, fn($q) => $q->whereIn('t.location_id', $permitted_locations))
            ->when(!empty($filters['location_id']), fn($q) => $q->where('t.location_id', (int)$filters['location_id']))
            ->when($start, fn($q) => $q->where('t.transaction_date', '>=', $start))
            ->when($end,   fn($q) => $q->where('t.transaction_date', '<',  $end))
            ->groupBy('variation_id', 't.location_id');

        $base->leftJoinSub($txWindow, 'txw', function($j){
            $j->on('txw.variation_id','=','variations.id')
                ->on('txw.location_id','=','vld.location_id');
        });

        // ---------- Purchase value aggregate (stock_price) ----------
        $purchaseAgg = DB::table('transactions as tp')
            ->selectRaw("
            pl.variation_id,
            tp.location_id,
            SUM(
              CASE
                WHEN tp.status='received'      THEN  (pl.quantity - COALESCE(pl.quantity_returned,0)) * pl.purchase_price_inc_tax
                WHEN tp.type='purchase_return' THEN -(pl.quantity) * pl.purchase_price_inc_tax
                ELSE 0
              END
            ) AS stock_price
        ")
            ->join('purchase_lines as pl', 'tp.id', '=', 'pl.transaction_id')
            ->when($restrict_locations, fn($q) => $q->whereIn('tp.location_id', $permitted_locations))
            ->when(!empty($filters['location_id']), fn($q) => $q->where('tp.location_id', (int)$filters['location_id']))
            ->when($start, fn($q) => $q->where('tp.transaction_date', '>=', $start))
            ->when($end,   fn($q) => $q->where('tp.transaction_date', '<',  $end))
            ->where(function($q){
                $q->where('tp.status', 'received')->orWhere('tp.type', 'purchase_return');
            })
            ->groupBy('pl.variation_id', 'tp.location_id');

        $base->leftJoinSub($purchaseAgg, 'pagg', function($j){
            $j->on('pagg.variation_id','=','variations.id')
                ->on('pagg.location_id','=','vld.location_id');
        });

        // ---------- Opening snapshot (< start) ----------
        if ($start) {
            $openAgg = DB::table('transactions as t')
                ->selectRaw("
                COALESCE(tsl.variation_id, sal.variation_id, pl.variation_id) as variation_id,
                t.location_id,
                SUM(CASE
                      WHEN t.status='received'      THEN  pl.quantity
                      WHEN t.type='purchase_return' THEN -pl.quantity
                      ELSE 0
                    END) AS open_purchases,
                SUM(CASE WHEN t.status='final' AND t.type='sell'          THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS open_sold,
                SUM(CASE WHEN t.status='final' AND t.type='sell_transfer' THEN  tsl.quantity                         ELSE 0 END) AS open_transferred,
                SUM(CASE WHEN t.type='stock_adjustment'                   THEN  sal.quantity                         ELSE 0 END) AS open_adjusted
            ")
            ...
            $base->leftJoinSub($openAgg, 'openx', function($j){
                $j->on('openx.variation_id','=','variations.id')
                    ->on('openx.location_id','=','vld.location_id');
            });
        }

        // ---------- Closing snapshot (< end) ----------
        if ($end) {
            $closeAgg = DB::table('transactions as t')
                ->selectRaw("
                COALESCE(tsl.variation_id, sal.variation_id, pl.variation_id) as variation_id,
                t.location_id,
                SUM(CASE
                      ...
                      WHEN t.type='purchase_return' THEN -pl.quantity
                      ELSE 0
                    END) AS close_purchases,
                SUM(CASE WHEN t.status='final' AND t.type='sell'          THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS close_sold,
                SUM(CASE WHEN t.status='final' AND t.type='sell_transfer' THEN  tsl.quantity                         ELSE 0 END) AS close_transferred,
                SUM(CASE WHEN t.type='stock_adjustment'                   THEN  sal.quantity                         ELSE 0 END) AS close_adjusted
            ")
                ->leftJoin('purchase_lines as pl', 't.id', '=', 'pl.transaction_id')
                ->leftJoin('transaction_sell_lines as tsl', 't.id', '=', 'tsl.transaction_id')
                ->leftJoin('stock_adjustment_lines as sal', 't.id', '=', 'sal.transaction_id')
            ...
            $base->leftJoinSub($closeAgg, 'closex', function($j){
                $j->on('closex.variation_id','=','variations.id')
                    ->on('closex.location_id','=','vld.location_id');
            });
        }
        ...
        COALESCE(txw.sold_qty,0)          as total_sold,
        COALESCE(txw.transferred_qty,0)   as total_transfered,
        COALESCE(txw.adjusted_qty,0)      as total_adjusted,
        COALESCE(pagg.stock_price,0)      as stock_price,
    ...
    ');
        ...
        if ($start) {
            ...
            (
              COALESCE(openx.open_purchases,0)
            - COALESCE(openx.open_sold,0)
            - COALESCE(openx.open_transferred,0)
            + COALESCE(openx.open_adjusted,0)
            ) AS opening_stock_as_of
        "));
        }
        if ($end) {
            $base->addSelect(DB::raw("
            (
              COALESCE(closex.close_purchases,0)
            - COALESCE(closex.close_sold,0)
            - COALESCE(closex.close_transferred,0)
            + COALESCE(closex.close_adjusted,0)
            ) AS closing_stock_as_of
        "));
        }

        $base->groupBy('variations.id', 'vld.location_id');

        // ---------- Manufacturing summary (optional) ----------
        if (!empty($filters['show_manufacturing_data'])) {
            $mfgAgg = DB::table('transactions as t')
                ->selectRaw("
                pl.variation_id, t.location_id,
                COALESCE(SUM(pl.quantity),0) as total_mfg_stock
            ")
            ...
            $base->leftJoinSub($mfgAgg, 'mfgx', function($j){
                $j->on('mfgx.variation_id','=','variations.id')
                    ->on('mfgx.location_id','=','vld.location_id');
            })->addSelect(DB::raw('COALESCE(mfgx.total_mfg_stock,0) as total_mfg_stock'));
        }

        // ---------- Stock status HAVING ----------
        if (!empty($filters['stock_status_id'])) {
            $status = $filters['stock_status_id'];
            if (defined('StockStatus::POS_NEG') && $status == StockStatus::POS_NEG) {
                $base->havingRaw('SUM(vld.qty_available) < 0 OR SUM(vld.qty_available) > 0');
            } else {
                $cmp = match ($status) {
                    StockStatus::NEGATIVE => '<',
                    StockStatus::POSITIVE => '>',
                ...
                if ($cmp) {
                    $base->havingRaw("SUM(vld.qty_available) $cmp 0");
                }
            }
        }
        ...
        // ---------- Per-product view ----------
        if (!empty($filters['product_id'])) {
            $base->where('p.id', $filters['product_id'])->groupBy('l.id');
        }

        // ---------- Group prices only when a single location is provided ----------
        if (!empty($filters['location_id'])) {
            $base->addSelect(DB::raw("
                (SELECT VGP.purchase_price_inc_tax
                   FROM variation_group_prices AS VGP
                   JOIN business_locations AS BL ON BL.selling_price_group_id = VGP.price_group_id
                  WHERE VGP.variation_id = variations.id AND BL.id = vld.location_id
                  LIMIT 1
                ) AS purchase_group_price
            "));
                    $base->addSelect(DB::raw("
                (SELECT VGP.sell_price_inc_tax
                   FROM variation_group_prices AS VGP
                   JOIN business_locations AS BL ON BL.selling_price_group_id = VGP.price_group_id
                  WHERE VGP.variation_id = variations.id AND BL.id = vld.location_id
                  LIMIT 1
                ) AS selling_group_price
            "));
        }
        else{
            $base->addSelect(DB::raw('NULL AS purchase_group_price'));
            $base->addSelect(DB::raw('NULL AS selling_group_price'));
        }

        // ---------- Return style ----------
        $base->orderBy('c.name')->orderBy('p.name');

        if ($for === 'view_product') {
            return $base->orderBy('l.name')->get();
        } elseif ($for === 'api') {
...
public function getProductStockDetails($business_id, $filters, $for)
{
    ...
    $tz = 'Africa/Nairobi'; // (+03:00). Use 'Africa/Dar_es_Salaam' if you prefer; both are +03:00.
    $start = !empty($filters['start_date'])
        ? \Carbon::parse($filters['start_date'], $tz)->startOfDay()->utc()
        : null;

    // If the user passed a full timestamp, keep that exact instant; otherwise use endOfDay.
    $end = null;
    if (!empty($filters['end_date'])) {
        $endInput = \Carbon::parse($filters['end_date'], $tz);
        $end = preg_match('/\d{2}:\d{2}/', (string)$filters['end_date'])
            ? $endInput->utc()
            : $endInput->endOfDay()->utc();
    }

    // ---- Access control / location scoping ----
    $permitted_locations = auth()->user()->permitted_locations();
    $restrict_locations  = is_array($permitted_locations) && $permitted_locations !== 'all';

    // ---- Base rows: (variation_id x location_id) ----
    // NOTE: INNER JOIN on vld to exclude non-located products.
    $base = DB::table('variations')
        ->join('products as p', 'p.id', '=', 'variations.product_id')
        ->join('units', 'p.unit_id', '=', 'units.id')
        ->join('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
    ...
    if (!empty($filters['location_id'])) {
        $locId = (int) $filters['location_id'];
        session(['location_id' => $locId]); // keep your original behavior
        $base->where('vld.location_id', $locId)
             ->join('product_locations as ploc', function ($j) use ($locId) {
                 $j->on('ploc.product_id', '=', 'p.id')
                   ->where('ploc.location_id', '=', $locId);
    ...
    if (isset($filters['only_mfg_products']) && (int)$filters['only_mfg_products'] === 1) {
        $base->join('mfg_recipes as mr', 'mr.variation_id', '=', 'variations.id');
    }
    if (isset($filters['active_state'])) {
        $base->where('p.is_inactive', $filters['active_state'] === 'inactive' ? 1 : 0);
    }
    if (isset($filters['not_for_selling']) && (int)$filters['not_for_selling'] === 1) {
        $base->where('p.not_for_selling', 1);
    }
    if (!empty($filters['repair_model_id'])) {
        $base->where('p.repair_model_id', $filters['repair_model_id']);
    ...
    // ---- Movements within window [start, end] (inclusive) ----
    $txWindow = DB::table('transactions as t')
        ->selectRaw("
            COALESCE(tsl.variation_id, sal.variation_id) AS variation_id,
            t.location_id,
            SUM(CASE WHEN t.status='final' AND t.type='sell'
                     THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS sold_qty,
            SUM(CASE WHEN t.status='final' AND t.type='sell_transfer'
                     THEN  tsl.quantity ELSE 0 END) AS transferred_qty,
            SUM(CASE WHEN t.type='stock_adjustment'
                     THEN  sal.quantity ELSE 0 END) AS adjusted_qty
        ")
        ->leftJoin('transaction_sell_lines as tsl', 't.id', '=', 'tsl.transaction_id')
        ->leftJoin('stock_adjustment_lines as sal', 't.id', '=', 'sal.transaction_id')
        // ->when($restrict_locations, fn($q) => $q->whereIn('t.location_id', $permitted_locations))
    ...
    // ---- Purchase value (stock_price) within window ----
    $purchaseAgg = DB::table('transactions as tp')
        ->selectRaw("
            pl.variation_id,
            tp.location_id,
        ...
        $openAgg = DB::table('transactions as t')
            ->selectRaw("
                ...
                SUM(CASE
                    ...
                    END) AS open_purchases,
                SUM(CASE WHEN t.status='final' AND t.type='sell'
                         THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS open_sold,
                SUM(CASE WHEN t.status='final' AND t.type='sell_transfer'
                         THEN  tsl.quantity ELSE 0 END) AS open_transferred,
                SUM(CASE WHEN t.type='stock_adjustment'
                         THEN  sal.quantity ELSE 0 END) AS open_adjusted
            ")
            ->leftJoin('purchase_lines as pl', 't.id', '=', 'pl.transaction_id')
            ->leftJoin('transaction_sell_lines as tsl', 't.id', '=', 'tsl.transaction_id')
            ->leftJoin('stock_adjustment_lines as sal', 't.id', '=', 'sal.transaction_id')
    ...
    // ---- Movements strictly AFTER end (for robust closing computation) ----
    if ($end) {
        $afterEnd = DB::table('transactions as t')
            ->selectRaw("
                COALESCE(tsl.variation_id, sal.variation_id) AS variation_id,
                t.location_id,
                SUM(CASE WHEN t.status='final' AND t.type='sell'
                         THEN (tsl.quantity - tsl.quantity_returned) ELSE 0 END) AS sold_after,
                SUM(CASE WHEN t.status='final' AND t.type='sell_transfer'
                         THEN  tsl.quantity ELSE 0 END) AS transferred_after,
                SUM(CASE WHEN t.type='stock_adjustment'
                         THEN  sal.quantity ELSE 0 END) AS adjusted_after
            ")
            ->leftJoin('transaction_sell_lines as tsl', 't.id', '=', 'tsl.transaction_id')
            ->leftJoin('stock_adjustment_lines as sal', 't.id', '=', 'sal.transaction_id')
            ->when($restrict_locations, fn($q) => $q->whereIn('t.location_id', $permitted_locations))
            ->when(!empty($filters['location_id']), fn($q) => $q->where('t.location_id', (int)$filters['location_id']))
            ->where('t.transaction_date', '>', $end) // strictly after the cutoff
            ->groupBy('variation_id', 't.location_id');

        $base->leftJoinSub($afterEnd, 'aft', function ($j) {
            $j->on('aft.variation_id','=','variations.id')
              ->on('aft.location_id','=','vld.location_id');
        });
    }
    ...
    if ($end) {
        // closing = current - movements strictly after $end (robust vs “now”)
        $base->addSelect(DB::raw('
            (
              SUM(vld.qty_available)
              - COALESCE(aft.sold_after,0)
              - COALESCE(aft.transferred_after,0)
              + COALESCE(aft.adjusted_after,0)
            ) AS closing_stock_as_of
        '));
    }

    // Always include group-price props (NULL when no single location)
    if (!empty($filters['location_id'])) {
        $base->addSelect(DB::raw("
            (SELECT VGP.purchase_price_inc_tax
               FROM variation_group_prices AS VGP
    ...
    if (!empty($filters['stock_status_id'])) {
        ...
        if (defined('StockStatus::POS_NEG') && $status == StockStatus::POS_NEG) {
            ...
            if ($cmp) $base->havingRaw("SUM(vld.qty_available) $cmp 0");
        }
    }
    ...
    public function getVariationStockDetails(array $filters)
    {
        ...
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                    ...
                    ->leftjoin('transactions as t', 'pl.transaction_id', '=', 't.id')
                       ->when($with_date == true, function ($query) use($start, $end) {
       ...
       public function getVariationStockHistory($filters)
    {
            ...
            'sl.transaction_id', '=', 'transactions.id')
                                ...
                                ->leftjoin('purchase_lines as rpl',
                                    'rpl.transaction_id', '=', 'return.id')
                                       ->leftjoin('stock_taking as sth',
                                    'sth.transaction_id', '=', 'transactions.id')
                                ...
                                ->where(function ($q) use ($variation_id) {
                                    $q->where('sl.variation_id', $variation_id)
                                        ->orWhere('pl.variation_id', $variation_id)
                                                                                ->orWhere('sth.variation_id', $variation_id)
                                    ...
                                    $query->whereDate('transactions.transaction_date', '>=', $start)
                                        ->whereDate('transactions.transaction_date', '<=', $end);
                                                            ...
                                                            ->whereIn('transactions.type', ['stock_taking','sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'sell_return', 'production_sell'])
                                ...
                                ->select(
                                    ...
                                    'pl.quantity as purchase_line_quantity',
                                                                        'sth.actual_quantity as product_counted',
        ...
        foreach ($stock_history as $stock_line) {
            ...
            if ($stock_line->transaction_type == 'sell') {
                ...
                $quantity_change = (is_null($stock_line->stock_taking_id) ? -1 : 1) * $stock_line->stock_adjusted;
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'stock_adjustment',
                    'type_label' => is_null($stock_line->stock_taking_id) ? __('stock_adjustment.stock_adjustment')  :  __('lang_v1.adjustment').__(' (Through Stock Taking)'),
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
               ...
               $to_transfer_location = Transaction::where('transfer_parent_id', $stock_line->transaction_id)->first()->location_id;
                $location = BusinessLocation::findOrFail($to_transfer_location);
                $quantity_change = -1 * $stock_line->sell_line_quantity;
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                ...
                $from_transfer_location = Transaction::where('id', $stock_line->transfer_parent_id)->first()->location_id;
                $location = BusinessLocation::findOrFail($from_transfer_location);
                $quantity_change = $stock_line->purchase_line_quantity;
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
              ...
              elseif ($stock_line->transaction_type == 'stock_taking') {
                $quantity_change = $stock_line->product_counted;
                $stock = $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $this->roundQuantity($quantity_change),
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'stock_taking',
                    'type_label' => __('lang_v1.stock_taking'),
                    'ref_no' => $stock_line->invoice_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit)
                ]);
            }
    ...
    public function getEvaluationByDepartment($business_id, $filters)
    {
        $query = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ->leftjoin('variation_location_details as vld', 'variations.id', '=', 'vld.variation_id')
            ->leftjoin('business_locations as l', 'vld.location_id', '=', 'l.id')
        ...
        $products = $query->select(
            DB::raw("SUM(vld.qty_available * variations.sell_price_inc_tax) as selling"),
            DB::raw("SUM(vld.qty_available * variations.dpp_inc_tax) as purchasing"),
            'c.name as category',
            'c.id as category_id'
        )->orderBy('c.name')->groupBy('c.id');
        return $products->get();
    }
}
