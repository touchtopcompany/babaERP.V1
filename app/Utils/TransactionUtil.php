...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if (empty($pay_term_number) || empty($pay_term_type)) {
         ...
         /*
         *---!!!!Check if the location is using default business or location currency
         *
         */
        $business = Business::findOrFail($business_id);
        $_multi_applicable = false;
        // if(multiCurrencyEnabled() && array_key_exists('enable_multi_currency', $business->common_settings)) {
        //     if(\Schema::hasColumn(table: 'business_locations', column: 'common_settings')){
        //         $_other_currency = BusinessLocation::findOrFail($input['location_id'])->common_settings;

        //         if(array_key_exists('location_currency_id', $_other_currency)){
        //             $_multi_applicable = true;
        //             $currency = $_other_currency['location_currency_id'];
        //             $multi = DB::table('multi_currencies_settings')->where('currency_id', $currency)->first();
        //             $input['exchange_rate'] = $multi->exchange_rate;
        //         }
        //     }
        ...
        $transaction = Transaction::create([
          ...
          if($_multi_applicable){
            $transaction->update(['for_curr' => $currency]);
        }
                    ...
                    foreach ($payments as $value) {
                        ...
                        if ($value['method'] == 'cash') {
                            ...
                            if ($value['is_return'] == 1) {
                                $output['change_return'] = ['method' => $value['method'], 'amount' => $this->num_f($value['amount'])
                                ];
                            }
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $purchase_sell_map = [];
            $stock = VariationLocationDetails::where([
            ...
            foreach ($rows as $k => $row) {
                ...
                if (! $allow_overselling && $qty_selling == 0 && $qty_allocated > 0 && $qty_allocated > $stock->qty_available + $qty_allocated){
                                        \Log::info('Logging 1');
                    $this->throwPurchaseMismatch(business: $business,line: $line , product: $product, mapping_type: $mapping_type, stop_selling_expired: $stop_selling_expired, qty_selling: -($stock->qty_available));
                }
            ...
            if (! ($qty_selling == 0 || is_null($qty_selling))) {
                ...
                if (! $allow_overselling) {
                    //MEANS THERE IS MISCALCULATIONS
                    //LETS CHECK ON THE STOCK TAKING
                    if(StockTaking::query()
                        ->join('transactions as TS', 'TS.id', 'stock_taking.transaction_id')
                        ->where([
                            'product_id' => $line->product_id,
                            'variation_id' => $line->variation_id,
                            'TS.business_id' => $business['id'],
                            'TS.location_id' => $business['location_id'],
                        ])->exists())
                    {

                        // $current_stock = VariationLocationDetails::query()
                        //     ->where([
                        //         'product_id' => $line->product_id,
                        //         'variation_id' => $line->variation_id,
                        //         'location_id' => $business['location_id']
                        ...
                        if ($stock->qty_available >= 0) {
                            ...
                            $qty_selling = 0;
                                                ...
                                                \Log::info('Logging 2');
                            $this->throwPurchaseMismatch(business: $business,line: $line , product: $product, mapping_type: $mapping_type, stop_selling_expired: $stop_selling_expired, qty_selling: $qty_selling);
                        }
                    }
                    else{
                                            \Log::info('Logging 3');
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false, $by_sale_price = false, $filters = [])
    {
        ...
        // ->where('purchase_lines.product_id', 35, 69)
        ...
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
                ...
                ->join(
                 ...
                 ->leftJoin(
                    'transactions AS SR',
                    function ($join) {
                        $join->on('transactions.id', '=', 'SR.return_parent_id')
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start, null, $location_id)
                ->select(
                    ...
                    DB::raw("SUM(IF(type = 'hms_booking' AND status = 'confirmed', final_total, 0)) as total_booking_invoice"),
        ...
        $prev_total_advance_payment = $this->__paymentQuery($contact_id, $start, null, $location_id)
         ...
         $prev_total_booking_invoice_paid = $prev_payments->where('transaction_type', 'hms_booking')->sum('amount');


        $total_prev_paid = $prev_total_invoice_paid + $prev_total_purchase_paid + $prev_total_booking_invoice_paid - $prev_total_sell_return_paid - $prev_total_purchase_return_paid + $prev_total_ob_paid + $prev_total_advance_payment;

        $total_prev_invoice =  $previous_transaction_sums->total_booking_invoice+ $previous_transaction_sums->total_purchase + $previous_transaction_sums->total_invoice - $previous_transaction_sums->total_sell_return - $previous_transaction_sums->total_purchase_return + $previous_transaction_sums->total_opening_balance - $previous_transaction_sums->total_ledger_discount;
        ...
        foreach ($transactions as $transaction) {
            ...
            $temp_array = [
                ...
                'total' => '',
                'payment_method' => '',
                'debit' => in_array($transaction->type, ['sell', 'purchase_return', 'hms_booking']) || ($transaction->sub_type == 'purchase_discount') ? $transaction->final_total : '',
        ...
        $invoice_sum = $transactions->where('type', 'sell')->sum('final_total');
                $booking_invoice_sum = $transactions->where('type', 'hms_booking')->sum('final_total');
        ...
        foreach ($payments as $payment) {
            ...
            $ledger[] = [
                ...
                'credit' => (in_array($payment->transaction_type, ['sell', 'purchase_return', 'opening_balance', 'hms_booking']) || ($payment->is_advance == 1 && in_array($contact->type, ['customer', 'both']))) && $payment->is_return == 0 || $payment->payment_type == 'credit' ? $payment->amount : '',
                'others' => $note,
            ];
        }
        ...
        $total_invoice_paid = ! empty($payments) ? $payments->where('transaction_type', 'sell')->where('is_return', 0)->sum('amount') : 0;
                $total_booking_invoice_paid = ! empty($payments) ? $payments->where('transaction_type', 'hms_booking')->sum('amount') : 0;
        ...
        $total_invoice = $invoice_sum - $sell_return_sum + $booking_invoice_sum;
        $total_purchase = $purchase_sum - $purchase_return_sum;

        $opening_balance_due = $opening_balance;
        $total_paid = $total_invoice_paid + $total_purchase_paid  + $total_booking_invoice_paid - $total_sell_return_paid - $total_purchase_return_paid + $total_excess_advance_payment - $total_advance_payment;

        $total_transactions_paid = $total_invoice_paid + $total_purchase_paid + $total_booking_invoice_paid - $total_sell_return_paid - $total_purchase_return_paid;
    ...
    public function getPurchaseOrderPdf($business_id, $transaction_id)
    {
        ...
        public function getTotalExpensePaid($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = TransactionPayment::join('transactions as t', function ($join){
        ...
        $total_expense_paid = $query->first()->total_paid ?? 0;

        return $total_expense_paid;
    }
    
    protected function throwPurchaseMismatch($business, $line, $product, $mapping_type, $stop_selling_expired, $qty_selling = null){
